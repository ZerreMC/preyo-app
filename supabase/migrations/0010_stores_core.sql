-- Stores: globals (seeded) + created by user.
-- is_global = true  → visible to all, only service_role can modify
-- is_global = false → created by user, only them can modify/delete
...
-- SELECT: all authenticated users can see all stores
CREATE POLICY stores_select ON public.stores
    FOR SELECT TO authenticated USING (true);

-- INSERT: only stores created by user
CREATE POLICY stores_insert_own ON public.stores
    FOR INSERT TO authenticated
    WITH CHECK (created_by = (SELECT auth.uid()) AND is_global = false);

-- UPDATE/DELETE: only own stores and not global ones
